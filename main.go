...
func main() {
	...
	for _, sub_result := range result.(*parsers.MultiParserResult).
		Results {
		...
		case *parsers.AssetList:
			for _, item := range r.Items {
				itemsMap[strings.ToLower(item.Name)] +=
					item.Quantity
			}
		...
		case *parsers.DScan:
			for _, item := range r.Items {
				itemsMap[strings.ToLower(item.Name)] += 1
			}
	...
	for name, quantity := range itemsMap {
		if quantity > 0 {
			items = append(
				items,
				OutItem{Name: name, Quantity: quantity},
			)
		}
	}
